...
function ChallengeSkeleton() {
  return (
    <div className="min-h-screen pb-16 animate-pulse" style={{ background: '#0D1117' }}>
      ...
      <main className="max-w-3xl mx-auto px-6 py-10 flex flex-col items-center gap-8">
        ...
        <div className="flex gap-8">
          <div className="w-56 h-80 rounded-xl" style={{ background: '#161B22' }} />
          <div className="w-56 h-80 rounded-xl" style={{ background: '#161B22' }} />
        </div>
      </main>
    </div>
...
export function ChallengePage() {
  ...
  if (error) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: '#0D1117' }}>
        <div className="text-center">
          <div className="font-display font-bold text-[24px] mb-2" style={{ color: '#E6EDF3' }}>Challenge unavailable</div>
          <div className="font-mono text-[13px] mb-6" style={{ color: '#8B949E' }}>{msg}</div>
          <Link to="/" className="font-mono text-[12px]" style={{ color: '#58A6FF' }}>← Generate your own report</Link>
        </div>
      </div>
    )
  }
  ...
  const challengedProfile = isAuthenticated && user
    ? {
  ...
  // Compute winner once accepted
  // For the challenged side we use their own commitCount from auth context.
  // Full integration: fetch their report for the same period. Sprint 9 scope:
  // show the card without full comparison until their report is fetched.
  const winner = accepted && challengedProfile
    ? determineWinner(challengerPayload.total_commits, challengerPayload.total_commits) // placeholder until challenged report fetched
    : null
    ...
    <div className="min-h-screen pb-16" style={{ background: '#0D1117' }}>
      ...
      <nav className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid #21262D' }}>
        <div className="flex items-center gap-2">
          ...
          <span className="font-display font-bold text-[14px]" style={{ color: '#E6EDF3' }}>GitReport</span>
        </div>
        <Link
          to="/"
          className="font-mono text-[12px] px-4 py-2 rounded-lg transition-all duration-150"
          style={{ background: '#161B22', color: '#8B949E', border: '1px solid #21262D', textDecoration: 'none' }}
        >
          Generate yours →
        </Link>
      </nav>
      ...
      <main className="max-w-3xl mx-auto px-6 py-10">
        ...
        <div className="text-center mb-10">
          <div className="font-mono text-[11px] uppercase tracking-widest mb-3" style={{ color: '#484F58', letterSpacing: '0.1em' }}>
            Challenge
          </div>
          <h1 className="font-display font-bold text-[28px] leading-tight mb-2" style={{ color: '#E6EDF3' }}>
            {username} is challenging you
          </h1>
          <p className="font-mono text-[13px]" style={{ color: '#8B949E' }}>
            {formatPeriod(period!)} · Can you beat {challengerPayload.total_commits} commits?
          </p>
        </div>
        ...
        <AnimatePresence>
          {winner && (
            ...
            >
              <Trophy size={20} weight="duotone" color={winner === 'tie' ? '#8B949E' : '#3FB950'} />
              <span className="font-mono text-[13px] font-medium" style={{ color: winner === 'tie' ? '#8B949E' : '#3FB950' }}>
                {winner === 'tie'
        ...
        <div className="flex items-start justify-center gap-8 flex-wrap mb-10">
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <ChallengeCard
              state="filled"
              payload={challengerPayload as unknown as AiPayload & { ai_summary: string; lines_added_total: number; prs_merged_total: number; repos_touched: number; daily_commits: number[] }}
            ...
            <AnimatePresence mode="wait">
              {accepted && challengedProfile ? (
                ...
                <motion.div key="blank" exit={{ opacity: 0, scale: 0.95 }}>
                  <ChallengeCard
                    state="blank"
                    label={isAuthenticated ? 'Generate your report' : 'Connect GitHub'}
        ...
        <div className="rounded-xl p-5" style={{ background: '#161B22', border: '1px solid #21262D' }}>
          <div className="font-mono text-[11px] uppercase tracking-widest mb-3" style={{ color: '#484F58', letterSpacing: '0.08em' }}>
            Share this challenge
          </div>
          <p className="font-mono text-[12px] mb-4 p-3 rounded-lg" style={{ color: '#8B949E', background: '#0D1117', border: '1px solid #21262D' }}>
            "{socialCaption}"
          </p>
